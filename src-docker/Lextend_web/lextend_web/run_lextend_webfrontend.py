    ...
    if version == OTA_LATEST_VERSION:
        ...
        def ota_and_reboot():
            ...
            try:
                ...
                ftp.set_pasv(True)  # Try with passive mode, else with active mode
                file = open(OTA_LATEST_FILENAME, 'wb')
                ftp.retrbinary('RETR %s' % OTA_LATEST_FILENAME, file.write)
                logger.info("Downloaded new package using passive mode.", exc_info=True)
            except:
                ftp = ftplib.FTP(OTA_SERVER, OTA_SERVER_LOGIN, OTA_SERVER_PASSWORD)
                ftp.set_pasv(False)
                file = open(OTA_LATEST_FILENAME, 'wb')
                ftp.retrbinary('RETR %s' % OTA_LATEST_FILENAME, file.write)
                logger.error("Could not download new package using passive mode, so "
                             "downloaded using active mode.", exc_info=True)
