...
class LextendEngine(object):
    ...
    #             sock.sendto(bytes(packet, "utf-8"), ("0.0.0.0", 5050))
    #     except:
    ...
    def socket_receiver(self):
        ...
        while True:
            ...
            #    continue
            try:
                ret = parseExtensionProtocol(data.decode("utf-8"), str(self.cfg))
                if ret:
...
def main():
    ...
    src_file = os.path.join(package_folder, "key.txt")
    with open(src_file) as f:
        content = f.readlines()

    for line in content:
        import hashlib
        line = hashlib.sha256(line.encode())

        if line.hexdigest() == LICENSE_KEY:
