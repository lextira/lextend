...
tockenfile = open('token.txt', 'w+')
sonosfile = open('sonos.txt', 'w+')
...
def io_access():
    auth_code = flask.request.args.get('accesscode')
    tockenfile = open('token.txt', 'w+')
    logger.info("ssonos")
    # auth_code = request.form["code"]
    refresh = refresh_token(auth_code)
...
def io_discover():
    ...
    sonosfile = open('sonos.txt', 'w+')
    sonosfile.truncate(0)

    access = tockenfile.readline()
    # access = request.json['data']
    household_id = discover(access)
    print '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$'
    print household_id
    # if household_id is not None:
    if household_id is not "No Device Found":
        headers = {
        ...
        if "errorCode" not in response_json:
            for i in range(len(response_json['players'])):
                sonosfile.write("group_name :{}\n".format(response_json['players'][i]['name'].encode("utf-8")))
            group_id = response_json["groups"][0]['id']
            print group_id
            vol = getVolume(access,household_id,group_id)
            vol =  str(vol)
            print "Volume: {}".format(vol)
            playbackState = response_json["groups"][0]['playbackState']
            sonosfile.write("group_id:{}".format(group_id) + "\n")
            sonosfile.write("household_id:{}".format(household_id) + "\n")
            sonosfile.close()
...
@app.route("/settings/group", methods=['GET', 'POST'])
def group():
    tockenfile = open('token.txt', 'r+')
    access = tockenfile.readline()
    tockenfile.close()
    ...
    group_sonos = flask.request.args.getlist('group_sonos_list[]')
    ungroup_sonos = flask.request.args.getlist('ungroup_sonos_list[]')

    logger.info(group_sonos)
    logger.info(ungroup_sonos)
    ...
    for s in player:
        if s in group_sonos:
            playerIDstoAdd.append(player[s].encode("utf-8"))
    print playerIDstoAdd
    for s in player:
        if s in ungroup_sonos:
            playerIDstoRemove.append(player[s].encode("utf-8"))
    print playerIDstoRemove

    addGroup(access,household_id, group_id, playerIDstoAdd, playerIDstoRemove)

    return ""

@app.route("/settings/setvolume", methods=['GET', 'POST'])
def setVolumes():
    # vol = request.form['sonosvolume']
    vol = flask.request.args.get('volume')
    # vol =  flask.request.json['volume']
    vol = str(vol)
    print vol
    logger.info("volume: {}".format(vol))
    ...
    print group_id
    sonosfile.close()
    res = setVolume(access, household_id, group_id,vol)
...
def AutoUpdate():
    ...
    if app.config["cfg"]:
        ...
        logger.info("Aut3"
                    "oUpdate : Auto update is disabled. Skipping auto update.")
