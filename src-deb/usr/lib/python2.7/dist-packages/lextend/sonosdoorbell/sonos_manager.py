...
from SoCo.soco import config
# from SoCo.soco.plugins.spotify import Spotify
# from SoCo.soco.plugins.spotify import SpotifyTrack
from SoCo.soco.data_structures import to_didl_string, DidlMusicTrack
from SoCo.soco.music_services import MusicService
...
class SonosPoolManager(object):
    ...
    def pause_play_bell_resume_sync(self, thread, uri, volume, groups, splitter, suffix, ignore_sonos_names_enabled):
        ...
        # Bell
        master.play_bell(thread, uri, volume)
    ...
    def pause_play_bell_resume(self, uri, volume=None, groups=None, splitter="", suffix="",
        ...
        if groups is None:
            groups = []
        return FuncThread(self.pause_play_bell_resume_sync, uri, volume, groups, splitter, suffix, ignore_sonos_names_enabled)

    def alarm_start(self, uri, volume):
        return FuncThread(self.alarm_start_sync, uri, volume)
...
class SonosDeviceManager(object):
    ...
    def save_state(self):
        ...
        try:
            self.state.media_uri = media_info['CurrentURI']
        except:
            self.logger.error("Could not get uri.", exc_info=True)

        try:
            self.state.media_metadata = media_info['CurrentURIMetaData']
        except:
        ...
        try:
            ...
            self.logger.error("Could not get uri.", exc_info=True)
        try:
            self.state.tittle = track_info['title']
        except:
            self.logger.error("Could not get title.", exc_info=True)
        try:
            self.state.metadata =track_info['metadata']
        except:
            self.logger.error("Could not get metadata.", exc_info=True)
        try:
    ...
    def pause_sync(self, thread, clear_q=True):
    ...
    def resume_sync(self, thread, no_clear_q):
            ...
            if reset_q:
                try:
                    ...
                    if uris:
                        ...
                        uris_chuncks = [uris[x:x+16] for x in xrange(0, len(uris), 16)]
        ...
        if self.device.is_coordinator:
            try:
                if self.state.from_queue:
                    ...
                    self.device.play_uri(self.state.media_uri, self.state.media_metadata)
    ...
    def resume(self, no_clear_q):
        ...
        """
        return FuncThread(self.resume_sync, no_clear_q)

    def pause(self, clear_q):
        """ Call pause_sync in a separate thread to make it async.
        """
        return FuncThread(self.pause_sync, clear_q)
