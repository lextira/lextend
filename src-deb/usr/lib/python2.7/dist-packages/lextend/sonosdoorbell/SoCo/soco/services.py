...
###################################################################################
    ...
    def handle_upnp_error(self, xml_error):
        ...
        if error_code is not None:
            ...
            # when there is no prev/next track to play
            if description == "Illegal seek target":
                logging.info("%s in %s" % ("End of playlist", self.soco.ip_address))
                return None
            # when the playlist is empty
            if description == "Transition not available":
                logging.info("%s in %s" % ("The playlist is empty", self.soco.ip_address))
                return None
